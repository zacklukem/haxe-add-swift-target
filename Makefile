...
ALL_CFLAGS=-thread -g $(CFLAGS) $(ALL_INCLUDES) $(FINDLIB_PACKAGES)
...
build_pass_4: $(MODULES:%=%.$(MODULE_EXT))
	$(COMPILER) -linkpkg -g -o $(OUTPUT) $(NATIVE_LIBS) $(NATIVE_LIB_FLAG) $(LFLAGS) $(FINDLIB_PACKAGES) $(EXTLIB_INCLUDES) $(EXTLIB_LIBS:=.$(LIB_EXT)) $(MODULES:%=%.$(MODULE_EXT))
